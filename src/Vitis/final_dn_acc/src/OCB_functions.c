...
enum OCB_status plainntext_to_blocks(char plaintext[], unsigned char*** Blocks, int blocks_number, int reminder);
...
enum OCB_status Tag_remover(char* ciphertext, char** plaintext, char** Tag);

void ciphertext_blocks_to_str(unsigned char** cipher_blocks, int blocks_number, int reminder, char chipertext[]);

char int_to_hex(int h);

void array_1d_to_str(unsigned char arr[], char str[]);

enum OCB_status OCB_init(int blocks_number, unsigned char round_keys[ROUND_KEYS_NUMBER][STATE_COLUMNS_NUMBER][STATE_COLUMNS_NUMBER], unsigned char*** Z_Array, unsigned char* L_inv_array, char* nonce);
...
void Number_of_Blocks(char* str, int* Blocks_number, int* reminder)
{
	...
	*reminder = len % BLOCK_BLEN;
	*Blocks_number = (*reminder == 0) ? (len / BLOCK_BLEN) : ((len / BLOCK_BLEN) + 1);			// if there is reminder increase the blocks number by one.
}
...
enum OCB_status zero_padding(char** str)
{
	...
	char* tmp_str = (char*)malloc((BLOCK_BLEN + 1) * sizeof(char));
	if (tmp_str == NULL)return MEMORY_ALLOCATING_ERROR;
	for (int i = 0; i < BLOCK_BLEN + 1; i++)
	{
...
enum OCB_status plainntext_to_blocks(char plaintext[], unsigned char*** Blocks, int blocks_number, int reminder)
{
	...
	unsigned char** tmp_blocks = (unsigned char**)malloc((blocks_number) * sizeof(unsigned char*));
	if (tmp_blocks == NULL)return MEMORY_ALLOCATING_ERROR;
	char* tmp_rem = NULL;
	if (tmp_blocks == NULL)return MEMORY_ALLOCATING_ERROR;
	int rounds = (reminder == 0) ? blocks_number : blocks_number - 1;
	for (int i = 0; i < rounds; i++)
	{
		*(tmp_blocks + i) = (unsigned char*)malloc(BYTE_TO_BIT * sizeof(unsigned char));
		if (*(tmp_blocks + i) == NULL)return MEMORY_ALLOCATING_ERROR;
		else
		{
			for (int j = 0; j < BLOCK_BLEN; j += 2)
			{
				tmp_blocks[i][j / 2] = (hex_to_int(plaintext[(BLOCK_BLEN * i) + j]) << 4) + hex_to_int(plaintext[(BLOCK_BLEN * i) + j + 1]);
			}
		}
	...
	if (reminder != 0)
	{
		tmp_blocks[blocks_number - 1] = (unsigned char*)malloc(BYTE_TO_BIT * sizeof(unsigned char));
		if (tmp_blocks[blocks_number - 1] == NULL)return MEMORY_ALLOCATING_ERROR;
		tmp_rem = (char*)malloc((reminder) * sizeof(char));
		if (tmp_rem == NULL)return MEMORY_ALLOCATING_ERROR;
		for (int i = 0; i < reminder; i++)
		{
			*(tmp_rem + i) = plaintext[BLOCK_BLEN * (blocks_number - 1) + i];
		}
		*(tmp_rem + reminder) = '\0';
		...
		tmp_blocks[blocks_number - 1] = (unsigned char*)malloc(BYTE_TO_BIT * sizeof(unsigned char));
		if (tmp_blocks[blocks_number - 1] == NULL)return MEMORY_ALLOCATING_ERROR;
		for (int i = 0; i < BLOCK_BLEN; i += 2)
		{
			tmp_blocks[blocks_number - 1][i / 2] = (hex_to_int(tmp_rem[i]) << 4) + hex_to_int(tmp_rem[i + 1]);
		}
		tmp_rem = NULL;
		free(tmp_rem);
	}
...
*/
void ciphertext_blocks_to_str(unsigned char** cipher_blocks, int blocks_number, int reminder, char ciphertext[])
{
	int i = 0, k = 0;
	int rounds = (reminder == 0) ? blocks_number : blocks_number - 1;
	for (; i < rounds; i++)
...
enum OCB_status OCB_init(int blocks_number, unsigned char round_keys[ROUND_KEYS_NUMBER][STATE_COLUMNS_NUMBER][STATE_COLUMNS_NUMBER], unsigned char*** Z_Array, unsigned char* L_inv_array, char* nonce)
{
	...
	unsigned char** L_Array = NULL;
	unsigned char** Z_Array_tmp = NULL;
	...
	status = L_Array_init(&L_Array, mu + 1);
	if (status != SUCESS)return ARRAY_INITIATING_ERROR;
	status = L_Array_init(&Z_Array_tmp, blocks_number + 1);
	if (status != SUCESS)return ARRAY_INITIATING_ERROR;
	...
	xor_arrays(L_Array[0], R_array, Z_Array_tmp[1], STR_TO_1D_ARRAY);						//Z(1)=L xor R
	for (int i = 2; i < blocks_number + 1; i++)
	{
		status = ntz(i, &ntz_i);
		if (status != FOUND)return NTZ_ERROR;
		xor_arrays(Z_Array_tmp[i - 1], L_Array[ntz_i], Z_Array_tmp[i], STR_TO_1D_ARRAY);	//Z(i)=Z(i-1) xor L(ntz(i))
	}
	*Z_Array = Z_Array_tmp;
	Z_Array_tmp = NULL;
	free(Z_Array_tmp);
	free_1D_array_memory(L_Array, mu);
	return SUCESS;
}
...
{
	char* rval;
	char* Tag_Tag = (char*)malloc((BLOCK_BLEN + 1) * sizeof(char));
	if (Tag_Tag == NULL)return MEMORY_ALLOCATING_ERROR;
	char* temp_plain = (char*)malloc((((int)strlen(ciphertext)) + 1 - BLOCK_BLEN) * sizeof(char));
	if (temp_plain == NULL)return MEMORY_ALLOCATING_ERROR;
	rval = strncpy(temp_plain, ciphertext, (int)strlen(ciphertext) - BLOCK_BLEN);						//copying from the begining to the last tau bits.
	temp_plain[(int)strlen(ciphertext) - BLOCK_BLEN] = '\0';
	rval = strncpy(Tag_Tag, ciphertext + ((int)strlen(ciphertext) - BLOCK_BLEN), BLOCK_BLEN);			//copying the last tau bits.
	if (rval == NULL)return STRING_COPY_ERROR;															//check for copying error.
	Tag_Tag[BLOCK_BLEN] = '\0';
...
{
	char* Tag = (char*)malloc((BLOCK_BLEN + 1) * sizeof(char));
	if (Tag == NULL)return MEMORY_ALLOCATING_ERROR;
	enum OCB_status status = SUCESS;
	enum status AES_status = PASS;
	int blocks_number = 0, reminder = 0;
	unsigned char** Blocks = NULL;
	unsigned char** Z_Array = NULL;
	unsigned char** C_Array = NULL;
	...
	if (ENC_OR_DEC == ENCRYPTION)
	{
		...
		{
			text_IN = (char*)malloc((BLOCK_BLEN+1) * sizeof(char));
			if (text_IN == NULL)return MEMORY_ALLOCATING_ERROR;
		...
		{
			text_IN = (char*)malloc((int)strlen(plain_or_cipher) * sizeof(char));		
			if (text_IN == NULL)return MEMORY_ALLOCATING_ERROR;
	...
	tmp_cipher_or_plain = (char*)malloc((strlen(text_IN) + 1) * sizeof(char));						//allocate memory for the result.
	int kk=strlen(text_IN) + 1;
	if (tmp_cipher_or_plain == NULL)return MEMORY_ALLOCATING_ERROR;
	...
	status = L_Array_init(&C_Array, blocks_number);													//initiate the result array with zeros (ciphertext array).
	if (status != SUCESS)return ARRAY_INITIATING_ERROR;
	...
	if (ENC_OR_DEC == ENCRYPTION)
	{
		*cipher_or_plain = (char*)malloc(((int)strlen(Tag) + (int)strlen(tmp_cipher_or_plain) + 1) * sizeof(char));
		strcpy(*cipher_or_plain, tmp_cipher_or_plain);												//Cc = C.
		strcat(*cipher_or_plain, Tag);																//Cc = C || T.
	}
	else
	{
		*cipher_or_plain = (char*)malloc(((int)strlen(tmp_cipher_or_plain) + 1) * sizeof(char));
	...
	text_IN = NULL;
	free(text_IN);
	tmp_cipher_or_plain = NULL;
	free(tmp_cipher_or_plain);
	free_1D_array_memory(Z_Array, blocks_number + 1);
	free_1D_array_memory(C_Array, blocks_number);
	return SUCESS;
}
